...
/**
 * **Concepto:** Las `sealed classes` también pueden ser genéricas. Esto es muy útil para
 * representar resultados de operaciones que pueden devolver diferentes tipos de datos.
 *
 * **Utilidad en Compose:** Cuando realizas operaciones asíncronas (como llamadas a API),
 * el resultado puede ser un éxito con datos o un error. 
 *
 * **Explicación:** En este ejemplo usamos un genérico simple `Result<T>`. 
 * Todas las subclases heredan ese tipo `T`. 
 * - `Success<T>` lleva los datos.
 * - `Error<T>` y `Loading<T>` no llevan datos de tipo T, pero mantienen el tipo genérico
 *   para que la estructura sea coherente.
 * 
 * *Nota:* En código profesional verás a menudo `Result<out T>`. El `out` permite que
 * un `Result<Nothing>` sea compatible con cualquier `Result<T>`, pero para empezar,
 * es más sencillo mantener el genérico en todas las subclases.
 */
...
fun ejercicio3ResultadoOperacion() {
    // Simulamos una función que devuelve un Result
